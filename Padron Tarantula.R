...
data_raw <- 
  read.csv("Tarantula RLC phos reinjects 2 3 4 5 11 16 plus Dros DB 5 miscleaves all peps.csv")
...
## Further narrow max peptides in normalized dataset
max_pep2 <- 3
data <-
  tbl_df(data) %>%
  group_by(Master.Protein.Accessions) %>%
  top_n(n = max_pep2, wt = ctrl_raw_med) %>%
  as.data.frame
...
gene_df <- read.csv('Tarantula RLC phos reinjects 2 3 4 5 11 16 plus Dros DB 5 miscleaves gene list.csv')
...
write.csv(protein, "Reinjects protein top 3 norm to MYH top 5 WIP.csv")
write.csv(data, "Reinjects top 3 peptides norm to MYH top 5 WIP.csv")
