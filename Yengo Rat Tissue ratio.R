...
# Andrea Thompson IPS samples

library(tidyverse)
source("C:/Users/PrevBeast/Documents/GitHub/Previs/proteomics_functions.R")

setwd("C:/Users/PrevBeast/Documents/R/Yengo/Rat PTMs/EtOH Study")
data_raw <- read.csv("Tissue 1 through 22 all peptides.csv")
...
group1 <- grep("EtOH", colnames(data)) %>%
  intersect(., normcols)
ctrl <- grep("Control", colnames(data)) %>%
  intersect(., normcols)
...
ctrlcols <- which(str_detect(colnames(data), "Control"))
g1cols <- which(str_detect(colnames(data), "EtOH"))
...
# #add two cols to HFpEF samples [RELIC FROM HFPEF SCRIPT THAT MAY BECOME VERY USEFUL]
data$dummy1_Control_norm_log <- NA
data$dummy2_Control_norm_log <- NA

group1_log_cols <- grep("EtOH_norm_log", colnames(data))
...
# Converting protein accession to gene symbol ----------------------------------
gene_df <- read.csv('Tissue 1 through 22 gene list.csv')
...
write.csv(protein, "Rat heart EtOH proteins Top3 with Ratio Sumtot Norm 5.csv")
write.csv(data, "Rat heart EtOH Top3 peptides Sumtot Norm 5.csv")
