...
dom_raw <- read.csv("Regions by NCBI exon 51 and 89 reapportioned.csv")
...
peps <- read.csv("Rbm20 rescue all titin peptides.csv")
...
controls <- grep("GFP", colnames(peps))
g1 <- grep("Rbm20", colnames(peps))
...
plot(x = peps$PepStart, peps$avg_control, ylim = c(0, 3E+8))
points(peps$PepStart, peps$avg_g1)

write.csv(peps, file = "Rbm annotated peptides corrected ranges2.csv")

# Taking in manually normalized values with summary ratio pre-calculated

normval_old <- read.csv("Renorm Aband WRT Cronos.csv")
normval <- read.csv("Rbm annotated peptides 226 300 norm.csv")
...
group1 <- grep("Rbm20", colnames(normval)) %>%
  intersect(., normcols)
ctrl <- grep("GFP", colnames(normval)) %>%
  intersect(., normcols)
...
normval$g1.control <- abun_ratio(normval, "group1_med")
...
normval <- normval[which(!is.na(normval$g1.control)),]

# Remove outliers --------------------------------------------------------------

pro_out1 <- by_variable(normval, "g1.control", VAR = "domain") %>% as.data.frame %>%
  rownames_to_column("domain")

normval <- rm_outliers_dom(normval, pro_out1, "g1.control")
...
regional$g1.control <- as.numeric(by_variable(normval, "g1.control", VAR = "domain"))

# # Adding grouped ratio if regions with NA ratio values are removed
# withnan <- as.numeric(by_variable(normval, "g1.control", VAR = "domain"))
# 
# regional$g1.control <- withnan[!is.nan(withnan)]

# Standard deviation relative protein abundance ratio
normval$g1.control_sd <- ratio_sd(normval, "g1.control", "group1_sd", "group1_med", 
                           "ctrl_sd", "ctrl_med")

# Standard deviation grouped relative protein abundance ratio1
normval$g1.control_df <- normval$group1_df + normval$ctrl_df
normval$g1.control_sd_df <- square_x_df(normval, "g1.control_sd", "g1.control_df")

# ratio1 sd_calc
sd_calc <- c("g1.control_df", "g1.control_sd_df")
temp_df <- by_variable(normval, sd_calc, FUN = sum, VAR = "domain") %>% as.data.frame

g1.control_sd <- temp_df$g1.control_sd_df / temp_df$g1.control_df
regional <- cbind(regional,  g1.control_sd)
# NOTE !!!! sd > 0 condition is only to make this work with NA exons (I.E., ratio undefined)
# regional <- cbind(regional,  g1.control_sd[g1.control_sd > 0])
...
group1_log_cols <- intersect(log_cols, grep("Rbm20", colnames(normval)))
ctrl_log_cols <- intersect(log_cols, grep("GFP", colnames(normval)))
...
write.csv(regional, "Rbm20 Regional abundance ratios 226 300 norm.csv")
write.csv(normval, "Rbm20 Regional peptide abundance ratios 226 300 norm.csv")
