...
setwd("C:/Users/PrevBeast/Documents/R/Andrea Thompson")
data_raw <- read.csv("Thompson IPS and CC3 cells All Peptides.csv")

# Normalization ----------------------------------------------------------------

ctrl_raw <- grep("CC3", colnames(data_raw))
...
group1 <- grep("IPS19", colnames(data)) %>%
  intersect(., normcols)
ctrl <- grep("CC3", colnames(data)) %>%
  intersect(., normcols)
...
ctrlcols <- which(str_detect(colnames(data), "CC3"))
IPScols <- which(str_detect(colnames(data), "IPS19"))
...
# Converting protein accession to gene symbol ----------------------------------
gene_df <- read.csv('Thompson IPS and CC3 cells Gene List.csv')
...
write.csv(protein, "Thompson IPS cells Top3 with Ratio Sumtot Norm.csv")
write.csv(data, "Thompson IPS cells Top3 peptides Sumtot Norm.csv")
