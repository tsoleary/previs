...
data_raw <- read.csv("Bmal KO Titin Splicing Genotype Corrected all peptides Gastroc only.csv")
...
KOcols <- which(str_detect(colnames(data), "KO"))
...
# Converting protein accession to gene symbol ----------------------------------
gene_df <- read.csv('Bmal KO Titin Splicing Genotype Corrected TA and Gastroc gene list.csv')
...
write.csv(protein, "Genotype corrected Bmal KO Gastroc Muscle Top3 with Ratio Sumtot Norm.csv")
write.csv(data, "Genotype corrected Bmal KO Gastroc Muscle Top3 peptides Sumtot Norm.csv")
